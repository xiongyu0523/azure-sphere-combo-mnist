...
void ili9341_fillCircleHelper(uint16_t x0, uint16_t y0, uint16_t r, uint8_t cornername, int16_t delta, uint16_t color) 
{
	if (r == 0) {
		return;
	}
	...
	while (x < y) {
		...
		if (cornername & 0x1) {
			ili9341_draw_v_Line(x0 + x, y0 - y, (uint16_t)(2 * y + 1 + delta), color);
			ili9341_draw_v_Line(x0 + y, y0 - x, (uint16_t)(2 * x + 1 + delta), color);
		}
		if (cornername & 0x2) {
			ili9341_draw_v_Line(x0 - x, y0 - y, (uint16_t)(2 * y + 1 + delta), color);
			ili9341_draw_v_Line(x0 - y, y0 - x, (uint16_t)(2 * x + 1 + delta), color);
...
void ili9341_fillCircle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color) {
	ili9341_draw_v_Line(x0, y0 - r, (uint16_t)(2 * r + 1), color);
